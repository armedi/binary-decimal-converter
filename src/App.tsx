...
function App() {
  ...
  const [conversionType, setConversionType] = useState<'binaryToDecimal' | 'decimalToBinary'>('binaryToDecimal');
  ...
  const convertDecimalToBinary = (decimal: number): string => {
    ...
    if (intPart > 0) {
      ...
      while (temp > 0) {
        intSteps.unshift(`${temp} ÷ 2 = ${Math.floor(temp / 2)} remainder ${temp % 2}`);
    ...
    if (fracPart > 0) {
      ...
      for (let i = 0; i < maxPrecision && fraction !== 0; i++) {
        steps.push(`  Step ${i + 1}: ${fraction} × 2 = ${(fraction * 2).toFixed(10)}`);
        fraction *= 2;
      ...
      if (fraction !== 0) {
        steps.push(`Note: Result truncated to ${maxPrecision} decimal places for binary fraction.`);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <h1 className="text-2xl font-bold mb-4 text-center">Binary-Decimal Converter</h1>
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 mb-2">Conversion Type</label>
          <div className="flex justify-center space-x-4">
            <button
              className={`px-4 py-2 rounded ${
                conversionType === 'binaryToDecimal' ? 'bg-blue-500 text-white' : 'bg-gray-200'
              }`}
            ...
            <button
              className={`px-4 py-2 rounded ${
                conversionType === 'decimalToBinary' ? 'bg-blue-500 text-white' : 'bg-gray-200'
              }`}
        ...
        <div className="mb-4">
          <label htmlFor="input" className="block text-sm font-medium text-gray-700 mb-2">
            Enter {conversionType === 'binaryToDecimal' ? 'Binary' : 'Decimal'} Number
          </label>
          <input
            type="text"
            id="input"
            ...
            placeholder={conversionType === 'binaryToDecimal' ? 'e.g., 1010.11' : 'e.g., 10.75'}
          />
        </div>
        <button
